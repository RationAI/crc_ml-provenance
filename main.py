...
from prunning.src_code import adapted_vgg
...
def main():
    ...
    train_loader.set_batch_size(args.batch_size)
    val_loader.set_batch_size(args.batch_size)
    ...
    model_ft = adapted_vgg.OnePrunableLayerAdaptedVGG16(args.layer_id, model_weight).cuda()  # Create a new network with a specific layer in mind
    print(model_ft)
    ...
    #print("ScaleFactorList", scale_factor_list, len(train_loader), args.num_epochs)
    reg_lambda = 10.0
...
def train(train_loader, model, criterion, optimizer, epoch, reg_lambda):
    ...
    for i in range(len(train_loader)):
        ...
        target = target.type(torch.FloatTensor).view(target.size(0), 1)
        ## torch async magic
        ...
        accuracy = binary_accuracy(output.round(), target_var.type(torch.int))
        losses.update(loss, input.size(0))
        bin_acc_avg.update(accuracy, input.size(0))
        ...
        if i % args.print_freq == 0:
            print('Epoch[{0}]: [{1}/{2}]\t'
                  ...
                  'BinAccuracy {bin_acc.val:.3f} ({bin_acc.avg:.3f})'.format(
...
def validate(val_loader, model, criterion, channel_index):
    with torch.no_grad():
        ...
        for i in range(len(val_loader)):
            ...
            # measure accuracy and record loss
            #prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
            accuracy_binary_batch = binary_accuracy(output.round(), target_var.type(torch.int))
            losses.update(loss, input.size(0))
            #top1.update(prec1[0], input.size(0))
            #top5.update(prec5[0], input.size(0))
            bin_acc_avg.update(accuracy_binary_batch)
...
def accuracy(output: torch.Tensor, target, topk=(1,)):
    ...
    #print("OUTPUT:", output, output.shape, maxk, batch_size)
...
def accuracy_binary_batch(output: torch.Tensor, target: torch.Tensor):
    return output.eq(target).sum(dim=0)
