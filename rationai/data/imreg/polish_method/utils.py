...
def pl_warp(image, out_shape, u_x_nr, u_y_nr ):
    y, x = out_shape[0], out_shape[1]
    x_new, y_new = resample_displacement_field(u_x_nr, u_y_nr, x, y)
    if (len(image.shape) == 2):
        return warp_image(image, x_new, y_new)
    a = []
    for k in range(3):
        a.append(warp_image(image[:, :, k], x_new, y_new))
