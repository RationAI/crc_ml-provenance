...
class MyCS(nn.Module):
    """
    Channel Selection Layer
    uses convolution layer and max pooling
    """
    def __init__(self, channel_depth, activation_size, kernel_size):
        super(MyCS, self).__init__()
        self.layer_type = 'MyCS'

        self.conv = nn.Conv2d(channel_depth, channel_depth,
                              kernel_size=int(activation_size / kernel_size), stride=1, padding=0)
        self.map = nn.MaxPool2d(kernel_size=kernel_size, stride=kernel_size)
        self.sigmoid = nn.Sigmoid()
        # self.conv.weight.data.normal_(0, 0.005)
        n = int(activation_size / kernel_size) * int(activation_size / kernel_size) * channel_depth
    ...
    def forward(self, x, scale_factor, channel_index=None):
        ...
        x_scale = self.map(x_scale)  # apply MAX Pooling: 1*512*14*14 => 1*512*7*7
